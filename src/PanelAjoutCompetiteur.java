					...
					for(Club c : Controleur.listClub){
						if(c.getNom().equals(list.getSelectedValue())){
							...
							if (!Controleur.verifInsertionCategoriePoids(poids,comboGenre.getSelectedItem().toString())){
								JOptionPane.showMessageDialog(getParent(),
										"Le poids ne correspond pas à une categorie, veuillez en créer une");		
								return;
							}
							/* verifie si une categorie correspond au competiteur */
							if(!Controleur.verifInsertionPoids(poids,comboGenre.getSelectedItem().toString(),age)){
								JOptionPane.showMessageDialog(getParent(),
										"pas de categorie");		
								return;
							}
							...
							Categorie categorieCombattant= Controleur.inserCombattantCategorie(nouveauCompetiteur);
							/* insertion du combattant dans une categorie de poids*/
							CategoriePoids insertionCategoriePoidsCombattant= Controleur.inserCategoriePoids(nouveauCompetiteur,categorieCombattant);
							
							if (categorieCombattant==null){
								JOptionPane.showMessageDialog(getParent(),
							...
							model.addRow(new Object[]{nom.getText(),prenom.getText(),age,anneeNaissance,comboGenre.getSelectedItem().toString(),categorieCombattant.getNom(),c.getNom(),poids});
							break;
						}	
